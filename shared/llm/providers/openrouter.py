"""OpenRouter provider using the OpenAI Python SDK (OpenRouter is OpenAI-compatible).
Ref: https://openrouter.ai/docs/guides/community/openai-sdk
"""
...
def _map_error(e: Exception) -> LLMError:
    ...
    if status == 402:
        return LLMValidationError("OpenRouter payment required")
    return LLMError(str(e))


class OpenRouter(BaseLLMClient):
    """OpenRouter chat completions via OpenAI SDK with base_url and api_key."""
    ...
    def __init__(
        ...
        *,
        ...
        self._api_key = api_key or (settings.OPENROUTER_API_KEY.get_secret_value() if settings.OPENROUTER_API_KEY else "")
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        if not self._api_key:
            raise LLMValidationError("OPENROUTER_API_KEY is required")
